...
export default {
  ...
  methods: {
    ...
    async getEntities() {
      this.entityList = await sp.get(`api/SysRolePrivilege/search?roleid=${this.data.id}&roleType=0`);
      this.menuList = await sp.get(`api/SysRolePrivilege/search?roleid=${this.data.id}&roleType=1`);
    }
  }
};
</script>
