...
nuget restore ".\src\server_dotnet\SixpenceStudio.Blog\SixpenceStudio.Blog.sln"

# Build Dotnet files
$buildException = MSBuild.exe ".\src\server_dotnet\SixpenceStudio.Blog\SixpenceStudio.Blog.sln"  /t:rebuild  /p:Configuration=Release
If (! $?) { Throw $buildException }
New-Item -ItemType directory ".\build\web\bin"
$release = ".\src\server_dotnet\build\*"
Copy-Item -Force -Recurse $release ".\build\web\bin\"
